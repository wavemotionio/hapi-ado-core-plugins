const pack = require('../../../package'),
    ...
    WebApi = require('../../azureDevOps/webApi')
    Boom = require('boom'),
    Joi = require('joi'),
...
exports.plugin = {
    ...
    register: async (server, options) => {
        ...
        server.auth.strategy(strategyName, 'cookie', {
            ...
            appendNext: true,
            isSecure: process.env.NODE_ENV != 'dev' ? true : false,
            clearInvalid: true,
            isHttpOnly: false,
        ...
        const login = async (request, h) => {
                ...
                if (request.auth.isAuthenticated) {
                    return h.redirect(landingPageRoute);
                }
                ...
                if (request.method === 'post') {
                    ...
                    if (authorized) {
                        ...
                        return { status: appConfig.loginUrl, token: request.headers.authorization};
                    } else {
                        return { status: null, token: request.headers.authorization};
                    }
                }
